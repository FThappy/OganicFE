...
const PasswordCriteria = (props: Props) => {
  ...
  const criteria = [
    ...
    { label: 'Contains special character', met: password && /[^A-Za-z0-9]/.test(password) }
  ];
...
const PasswordStrengthMeter = (props: Props) => {
  ...
  const getStrength = (pass: string) => {
    let strength = 0;
    if (pass && pass.length >= 6) strength++;
    if (pass && pass.match(/[a-z]/) && pass.match(/[A-Z]/)) strength++;
    if (pass && pass.match(/\d/)) strength++;
    if (pass && pass.match(/[^a-zA-Z\d]/)) strength++;
    return strength;
  };
