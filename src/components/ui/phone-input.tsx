...
>(({ className, onChange, ...props }, ref) => {
  return (
    <RPNInput.default
      ref={ref}
      className={cn('flex', className, 'border-none')}
      flagComponent={FlagComponent}
      countrySelectComponent={selectProps => <CountrySelect {...selectProps} className={className} />}
      input
      inputComponent={inputProps => <InputComponent {...inputProps} className={cn(inputProps.className, className)} />}
      smartCaret={false}
      /**
...
const InputComponent = React.forwardRef<HTMLInputElement, InputProps>(({ className, ...props }, ref) => (
  <Input
    className={cn(`rounded-r-[8px] rounded-l-0 px-[14px] py-[10px] h-[52px] bg-white `, className)}
    {...props}
    ref={ref}
  />
));
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          type='button'
          variant={'outline'}
          className={`flex gap-1 rounded-e-none rounded-s-lg  focus:z-10 rounded-l-[8px] px-[14px] py-[10px] h-[52px] bg-white ${className} border-r-0`}
          disabled={disabled}
        >
